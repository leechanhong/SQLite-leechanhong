...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.ItemViewHolder> {
    //android에서 제공되는 OnItemClick리스너는 매개변수 4개 이기 때문에 2개짜리로 추가
    ...
    public ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        //레이아웃 xml을 자바단에서 화면을 로딩할때 inflater메서드 사용(래)=startActivity(화면을 띄울때)
    ...
    //개발자가 호출하는 것이 아니고, 안드로이드의 레이아웃 관리자프로그램이 자동으로 호출합니다.
    @Override
